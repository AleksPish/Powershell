...
#>
<# ----------------------------------------------- MANUAL INSPECTION ---------------------------------------------------------
        To inspect the FSLogix disks without running any maintenance, run the below command in PowerShell replacing both <share> and <csvfile.csv> with the correct values. You can then open the csv file in excel.

        Get-ChildItem -Path <share> -Filter "*.vhd*" -Recurse -File | Select-Object Name, @{n = 'SizeInGB'; e = {[math]::round($_.length/1GB,2)}} | Export-Csv -Path <csvfile.csv>
#>
<# ----------------------------------------------- PARAMETERS ----------------------------------------------------------------
        .PARAMETER Path
        ...
        If a disk â€˜last access timeâ€™ is older than todays date minus this value, the disk will be deleted from the share.  This is a permanent action.

        .PARAMETER LogFilePath
        All disk actions will be saved in a csv file for admin reference.  The default location for this csv file is the userâ€™s temp directory.  The default filename is in the following format: FslShrinkDisk 2020-04-14 19-36-19.csv
...
#>
<# ----------------------------------------------- HOW TO USE - MANUAL TASK --------------------------------------------------

    Below are examples for how to use this PowerShell script for running the task manually.
...
#>
<# ----------------------------------------------- HOW TO USE - SCHEDULED TASK -----------------------------------------------

    The configuration of this scheduled task conducts the following maintenance:
        * Finds all the .VHD* files in the fslogix_office share.
        * Ignores all .VHD* files less than 5 GB.
        * Deletes all .VHD* files that were last modified over 30 days ago.
        * Only runs the maintenance on the remaining .VHD* files if the whitespace is more than 15%.
        * Outputs a log file in the fslogix_office\logs folder. Refer to LOGGING to learn how to read the generated log file.


    This PowerShell script should be located on the server where the disks are stored, in the folder "x:\path\to\fslogix_office\logs". The desired configuration for a scheduled task is as follows:

        Name: FSLogix_Office_Disk_Shrink
        Description: This scheduled task runs the PowerShell script Invoke-FslShrinkDisk.ps1. WARNING! Do not run this task during office hours unless absolutely necessary as it will impact storage performance.

        Security options:
            User: The selected account should be the Local Administrator which should have "Full Control" permissions to the share "fslogix_office"
            The task should "Run whether user is logged in or not"
        ...
        Actions:
            ...
            Arguements: -ExecutionPolicy Bypass -File "I:\fslogix_office\logs\Invoke-FslShrinkDisk.ps1" -Path "I:\fslogix_office" -Recurse -IgnoreLessThanGB 5 -DeleteOlderThanDays 30 -RatioFreeSpace 0.15 -LogFilePath "I:\fslogix_office\logs"

        Settings:
            Enable: "Allow task to be run on demand."
            Enable: "If the running task does not stop when requested, force it to stop"
            Change: If the task is already running, then the following rule applies to: "Do not start a new instance"
#>
    [CmdletBinding()]
